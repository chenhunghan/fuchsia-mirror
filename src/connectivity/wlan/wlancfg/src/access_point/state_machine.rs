            ...
            Status::Started { band, channel, mode, num_clients, security_type } => {
                inspect_insert!(writer, var key: {
                    Started: {
                        ...
                        security_type: format!("{:?}", security_type)
                    }
                })
            }
            ...
            status_response = pending_status_req.select_next_some() => {
                ...
                match status_response.running_ap {
                    Some(sme_state) => {
                        ...
                        deps.state_tracker.consume_sme_status_update(cbw, req.radio_config.channel.band, *sme_state)
                            .map_err(|e| { ExitReason(Err(e)) })?;
                    }
                    None => {
                        deps.state_tracker.update_operating_state(types::OperatingState::Failed)
...
mod tests {
    ...
    fn test_stop_during_started() {
        ...
        let radio_config = new_radio_config();
        let req = ApConfig {
    ...
    #[fuchsia::test]
    fn test_stopping_status_inspect_log() {
    ...
    #[fuchsia::test]
    fn test_stopped_status_inspect_log() {
        let exec = fasync::TestExecutor::new_with_fake_time();
        let mut test_values = InspectTestValues::new(exec);
    ...
    #[fuchsia::test]
    fn test_starting_status_inspect_log() {
        let exec = fasync::TestExecutor::new_with_fake_time();
        let mut test_values = InspectTestValues::new(exec);
    ...
    #[fuchsia::test]
    fn test_started_status_inspect_log() {
        let exec = fasync::TestExecutor::new_with_fake_time();
        let mut test_values = InspectTestValues::new(exec);
        let hierarchy = test_values.log_status(Status::Started {
            band: types::OperatingBand::Any,
            channel: 1,
            mode: types::ConnectivityMode::Unrestricted,
            num_clients: 2,

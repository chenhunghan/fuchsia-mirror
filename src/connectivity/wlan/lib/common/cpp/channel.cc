...
bool IsValidChan2Ghz(const Channel& channel) {
  if (channel.primary.band != wlan_ieee80211_wire::WlanBand::kTwoGhz) {
    return false;
  }
...
bool IsValidChan5Ghz(const Channel& channel) {
  if (channel.primary.band != wlan_ieee80211_wire::WlanBand::kFiveGhz) {
    return false;
  }
...
std::string ChanStr(const Channel& channel) {
  char buf[8 + 1];
  if (channel.bandwidth != wlan_ieee80211_wire::ChannelBandwidth::kCbw80P80) {

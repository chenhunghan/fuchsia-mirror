...
pub struct MockNode {
    ...
    msg_response_pairs: RefCell<VecDeque<(MessageMatcher, Result<MessageReturn, CpuManagerError>)>>,
...
impl MockNode {
    ...
    pub fn add_msg_response_pair(
        &self,
        pair: (MessageMatcher, Result<MessageReturn, CpuManagerError>),
    ) {
...
impl MockNodeMaker {
    ...
    pub fn make(
        &mut self,
        name: &'static str,
        msg_response_pairs: Vec<(MessageMatcher, Result<MessageReturn, CpuManagerError>)>,
    ) -> Rc<MockNode> {
...
mod tests {
    ...
    fn test_msg_return_macros() {
        ...
        let ret: Result<MessageReturn, CpuManagerError> = msg_ok_return!(GetNumCpus(4));
        match ret {

...
std::unique_ptr<UsbAudioStreamInterface> UsbAudioStreamInterface::Create(
  ...
  if (ac.check()) {
    ...
    LOG_EX(ERROR, *parent,
           "Failed to add initial interface (id %u) to UsbAudioStreamInterface (res %d)", iid, res);
  } else {
...
zx_status_t UsbAudioStreamInterface::AddInterface(DescriptorListMemory::Iterator* iter) {
  ...
  ZX_DEBUG_ASSERT(ihdr->b_interface_number == iid());
    ...
    if (!formats_.is_empty()) {
      if (format->term_link() != term_link_) {
        LOG(WARNING,
            ...
            "(expected %u, got %u)",
            ihdr->b_interface_number, alt_id, term_link_, format->term_link());
        return ZX_OK;
      }
      ...
      if ((format->ep_addr() != ep_addr_) || (format->ep_attr() != ep_attr_)) {
        LOG(ERROR,
            ...
            ihdr->b_interface_number, alt_id, ep_addr_, ep_attr_, format->ep_addr(),
            format->ep_attr());
        return ZX_OK;
      }
    } else {
...
zx_status_t UsbAudioStreamInterface::BuildFormatMap() {
  if (format_map_.size()) {
    LOG(WARNING, "Attempted to re-build format map for streaming interface (iid %u)", iid());
  ...
  if (!ac.check()) {
    LOG(ERROR, "Out of memory attempting to reserve %zu format ranges", worst_case_map_entries);
    return ZX_ERR_NO_MEMORY;
  }
  ...
  for (const auto& fmt : formats_) {
    ...
    if (tag == USB_AUDIO_AS_FT_PCM8) {
      ...
      LOG(WARNING, "Skipping unsupported format tag (%u)", tag);
      continue;
    }
  ...
  if (format_map_.is_empty()) {
    LOG(WARNING, "Failed to find any usable formats for streaming interface (iid %u)", iid());
...
zx_status_t UsbAudioStreamInterface::ActivateFormat(size_t ndx, uint32_t frames_per_second) {
  ...
  if (!single_rate) {
    ...
    if (status != ZX_OK) {
      ...
      LOG(ERROR, "Failed to set frame rate %u for ep address %u (status %d)", frames_per_second,
          f.ep_addr_, status);
...
zx_status_t UsbAudioStreamInterface::ActivateIdleFormat() {
  ...
  ZX_DEBUG_ASSERT(idle_hdr_->b_interface_number == iid());
  return usb_set_interface(&parent_.usb_proto(), iid(), idle_hdr_->b_alternate_setting);
}
...
zx_status_t UsbAudioStreamInterface::Format::Init(DescriptorListMemory::Iterator* iter) {
  ...
  switch (class_hdr_->wFormatTag) {
    ...
    default:
      LOG(ERROR,
          ...
          "(iid %u, alt_id %u)",
          class_hdr_->wFormatTag, interface_hdr_->b_interface_number, alt_id());
      return ZX_ERR_NOT_SUPPORTED;
  }
    ...
    if (hdr->b_descriptor_type == USB_AUDIO_CS_INTERFACE) {
      ...
      if (fmt_hdr->bFormatType != USB_AUDIO_FORMAT_TYPE_I) {
        LOG(ERROR,
            ...
            fmt_hdr->bFormatType, interface_hdr_->b_interface_number, alt_id());
        return ZX_ERR_NOT_SUPPORTED;
      }
      ...
      if ((fmt_desc_ != nullptr) || (fmt_desc == nullptr)) {
        LOG(ERROR,
            ...
            "interface (iid %u, alt_id %u)",
            interface_hdr_->b_interface_number, alt_id());
        return ZX_ERR_NOT_SUPPORTED;
      }
      ...
      if (ep_desc_ != nullptr) {
        ...
        if ((usb_ep_type(ep_desc) != fuchsia_hardware_usb_descriptor::EndpointType::kIsochronous) ||
          ...
          LOG(WARNING,
              ...
              interface_hdr_->b_interface_number, alt_id(), ep_desc->bm_attributes);
        } else {
          ep_desc_ = ep_desc;
        }
      }
  ...
  if (expected_bytes != extra_bytes) {
    LOG(ERROR,
        ...
        interface_hdr_->b_interface_number, alt_id(), expected_bytes, extra_bytes);
  ...
  if ((frame_rate_cnt() == 0) && (min_cont_frame_rate() > max_cont_frame_rate())) {
    LOG(ERROR,
        ...
        "audio interface (iid %u, alt_id %u).",
        min_cont_frame_rate(), max_cont_frame_rate(), interface_hdr_->b_interface_number, alt_id());
    return ZX_ERR_INTERNAL;
  }

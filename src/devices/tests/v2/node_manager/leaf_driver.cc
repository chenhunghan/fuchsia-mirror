 ...
 private:
  zx::result<uint32_t> GetNumber(std::string_view instance) {
    ...
    if (device.status_value() != ZX_OK) {
      fdf::warn("Failed to connect to {}: {}", std::string(instance), device);
      return device.take_error();
    }

    auto result = fidl::WireCall(*device)->GetNumber();
    if (result.status() != ZX_OK) {
      fdf::warn("Failed to call number on {}: {}", std::string(instance),
  ...
  zx::result<> DoWork(const fidl::WireSharedClient<ft::Waiter>& waiter) {
    // Check the left device (which is a provided resource).
    auto number = GetNumber("left");
    if (number.is_error()) {
      fdf::error("Failed to get number for left: {}", number.status_string());
      [[maybe_unused]] auto result = waiter->Ack(number.error_value());
      return number.take_error();
    }
    ...
    // Check the right device (which is a provided resource).
    number = GetNumber("right");
    if (number.is_error()) {
      fdf::error("Failed to get number for right: {}", number.status_string());
      [[maybe_unused]] auto result = waiter->Ack(number.error_value());
      return number.take_error();
    }
    if (*number != 2) {

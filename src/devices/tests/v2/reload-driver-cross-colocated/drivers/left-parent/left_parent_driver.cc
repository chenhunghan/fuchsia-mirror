...
class LeftParentDriver : public fdf::DriverBase2 {
 public:
  LeftParentDriver() : fdf::DriverBase2("left_parent") {}

  zx::result<> Start(fdf::DriverContext context) override {
    auto incoming_ptr = std::shared_ptr<fdf::Namespace>(context.take_incoming());
    node_client_.Bind(take_node());

    zx::result result = helpers::AddChild(logger(), "child_a", node_client_,
                                          bindlib::TEST_BIND_PROPERTY_TARGET_1, "shared-host");
    if (result.is_error()) {
...
FUCHSIA_DRIVER_EXPORT2(LeftParentDriver);

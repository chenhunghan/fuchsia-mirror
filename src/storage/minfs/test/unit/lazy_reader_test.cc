...
TEST(LazyReaderTest, ReadSucceeds) {
  ...
  ASSERT_OK(bcache_or->Writeblk(static_cast<blk_t>(device_range.block()), data));

  // Now read the data back using the lazy_reader.
  storage::ResizeableVmoBuffer buffer(kMinfsBlockSize);
  ASSERT_OK(buffer.Attach("LazyReaderTest", **bcache_or));
  auto detach = fit::defer([&]() { [[maybe_unused]] auto _ = buffer.Detach(**bcache_or); });
...
TEST(LazyReaderTest, RunRequestsError) {
  ...
  EXPECT_STATUS(lazy_reader.Read(ByteRange(530, 531), &reader), ZX_ERR_IO);
  EXPECT_FALSE(reader.run_requests_called());
  reader.Reset();

    ...
    D: Pouf,
{
    ...
    pub fn new(url: Url, client: Client<C, Body>) -> Self {
    ...
    /// Create a new repository with the given `Url` and `Client`.
    pub fn new_with_uri(uri: Uri, client: Client<C, Body>) -> Self {
        HttpRepositoryBuilder {
            uri,
            client,
            user_agent: None,
    ...
    D: Pouf,
{
    fn get<'a>(
        &self,
        uri: &'a Uri,
    ) -> Result<impl Future<Output = Result<Response<hyper::body::Incoming>>> + 'a> {
    ...
    fn fetch_metadata<'a>(
        ...
        async move {
            ...
            if status == StatusCode::OK {
                let reader = http_body_util::BodyStream::new(resp.into_body())
